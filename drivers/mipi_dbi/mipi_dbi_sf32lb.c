...
static void mipi_dbi_sf32lb_spi_read_bytes(const struct device *dev, uint32_t addr, uint32_t addr_len,
...
static int mipi_dbi_sf32lb_8080_cmd_read_bytes(const struct device *dev, uint8_t *cmd, size_t num_cmds,
					...
					size_t num_cmds, uint8_t *response, size_t len)
{
	...
	case MIPI_DBI_MODE_SPI_4WIRE:
		...
		mipi_dbi_sf32lb_spi_read_bytes(dev, addr, sizeof(addr), response, len);
		break;
	default:
		return -EINVAL;
	}
